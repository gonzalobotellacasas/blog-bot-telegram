    ...
    if os.path.exists(STATE_FILE):
        try:
            with open(STATE_FILE, 'r', encoding='utf-8') as f:
                raw = json.load(f)
                conversaciones = {int(k): v for k, v in raw.items()}
                logger.info(f'Estado cargado: {len(conversaciones)} conversacion(es) en curso')
        except Exception as e:
            logger.error(f'No se pudo cargar el estado previo, empiezo de cero: {e}')
            conversaciones = {}
        else:
            conversaciones = {}
...
def guardar_estado():
    try:
        with open(STATE_FILE, 'w', encoding='utf-8') as f:
            json.dump(conversaciones, f, ensure_ascii=False)
    except Exception as e:
...
logger.error(f'Error generando artículo con Claude: {e}')
await context.bot.send_message(
    chat_id=query.message.chat_id,
    text="⚠️ Hubo un error generando el artículo con la IA. Escribe /start y vuelve a mandarme el tema y las respuestas dentro de unos minutos."
)
return
...
def main():
    ...
    logger.info('Bot iniciado v3 (con persistencia de estado)')
    app.run_polling()
